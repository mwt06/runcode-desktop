...
- unknown tools and unknown operations are denied.
- future mutating or command execution operations are modeled as approval-requiring, but without an interactive authorizer they resolve to denial.
...
Path resolution is shared through `internal/toolpath` so `Read` execution and permission resolution use the same base-directory semantics. Permission containment checks resolve existing symlink targets before deciding whether a read is inside the workspace.
