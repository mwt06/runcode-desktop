...
- `internal/persistence/sessions`: full session-history persistence behind a `Backend` interface (writable per-session `Store` plus `LoadHistory`/`List`/`Describe`/`Latest`), with two backends — the default append-only one-`.jsonl`-file-per-session store, and an indexed pure-Go SQLite database (`modernc.org/sqlite`, `<workspace>/.runcode/sessions.db`, schema versioned via `PRAGMA user_version`) selectable with `--session-backend sqlite`. Both back `--resume`/`--continue`, the `runcode sessions` command, and the TUI startup picker.
...
Transcript recording is opt-in through `--transcript jsonl|sqlite` or `RUNCODE_TRANSCRIPT`. The JSONL backend writes one whitelisted turn summary per successful turn under `<workspace>/.runcode/transcripts/<session-id>.jsonl`; the SQLite backend writes the same sanitized records into a single `<workspace>/.runcode/transcripts.db` (full record JSON plus denormalized session/time/model/user/assistant/tool columns and an FTS5 trigram index over the text) that the `runcode transcript list`/`search` command reads. `search` matches user, assistant, and tool text (tool names + Bash commands; `--tool` restricts to those) via the trigram index, falling back to LIKE for queries shorter than 3 codepoints. Failed turns are not written. If no session id is provided, runcode generates one. `/clear` only resets in-memory history and does not delete or rotate the transcript.
...
| `internal/persistence/transcript` | JSONL append behavior, session id validation, transcript sanitizer whitelist, and the SQLite recorder (record/search/list, FTS5 + LIKE-fallback search, tool-command and CJK search, v1→v2 migration backfill, reopen persistence, wildcard escaping) |
