...
export function MemoryPage() {
  ...
  const memSection = (title: string, note: string, raw: string[] | null | undefined) => {
    ...
    <div>
      <div className="flex items-baseline gap-2 mb-1.5">
        ...
        <span className="text-[12px] text-faint ml-auto">{note}</span>
      </div>
      {entries.length === 0 ? (
        <div className="text-faint text-[13px] py-2">暂无</div>
      ) : (
        <ul className="flex flex-col gap-1.5 m-0 p-0 list-none">
          {entries.map((e, i) => (
            <li key={i} className="text-[13px] text-ink bg-surface2 border border-line2 rounded-lg px-3 py-2 leading-relaxed break-words">{e}</li>
          ))}
        </ul>
      )}
    </div>
  ...
  return (
    <PageShell title="记忆与项目指令" hint="项目指令随每次对话注入给模型;记忆由模型在跨会话时自行记录。">
        ...
        {loading ? (
          ...
          <>
            <section className="bg-surface border border-line2 rounded-card p-5 flex flex-col gap-3 shadow-xs">
              <div className="flex items-center gap-2">
                <span className="text-[14px] font-semibold text-ink">项目指令</span>
                {ctx && <span className="font-mono text-[12px] text-faint bg-surface2 border border-line2 rounded px-1.5 py-0.5">{ctx.name}{!ctx.exists ? ' · 新建' : ''}</span>}
              ...
              {hasWorkspace ? (
                <textarea
                  className="font-mono text-[13px] leading-relaxed bg-surface2 text-ink border border-line2 rounded-field px-3 py-2.5 outline-none focus:border-primary focus:shadow-focus min-h-[240px] resize-y"
                  value={content}
            ...
            <section className="bg-surface border border-line2 rounded-card p-5 flex flex-col gap-4 shadow-xs">
              <div className="text-[14px] font-semibold text-ink">记忆</div>
              {memSection('用户记忆', '全局 · 跨项目', mem.user)}
              {memSection('项目记忆', '仅本工作区', mem.project)}
              <div className="text-[12px] text-faint">记忆由模型通过记忆工具自动维护;如需修改,可在对话中让它更新。</div>
            </section>
          </>
        )}
    </PageShell>
