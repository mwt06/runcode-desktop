...
| `HScroll` | 「一行放不下就横向滚」的轨道：藏滚动条、滚轮翻成横向、两端按那一侧还有没有内容画出淡出与翻页箭头。**藏了滚动条就必须把溢出画出来**——切断点落在两个 chip 之间时右边什么都不剩，整排看起来就是"只有这几项"，用户不会想到去滚。淡出用 mask 不用渐变：渐变要求调用方背景色与写死的那个色永远一致。三条横向条都走它——输入框上方的场景条、计划模式阶段进度条、预览标签条，**别再裸用 `overflow-x-auto`** |
