...
// reliably win by class-string order, so an un-flagged bg-primary lost to
// bg-surface and the primary button rendered white-on-white.
...
// SkillsPage is the project skill manager: browse, create, edit, and delete the
// workspace's `.runcode/skills/<name>/SKILL.md` files.
function SkillsPage({ onUse }: { onUse: (name: string) => void }) {
...
function AgentsPage({ onUse }: { onUse: (name: string) => void }) {
...
function SettingsPage({ initial, info, onSaved }: { initial: Partial<StartSessionRequest>; info: SessionInfo | null; onSaved: (info: SessionInfo) => void }) {
...
function StartForm({ onStart, starting, error, initial }: { onStart: (req: StartSessionRequest) => void; starting: boolean; error: string; initial: StartSessionRequest }) {
