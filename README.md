...
- Slash commands run on an extensible registry (built-ins `/help`, `/clear`, `/status`, `/exit`), but functional commands like `/compact`, `/model`, and `/cost` are not implemented yet; no MCP, hooks, sub-agents, or skills.
